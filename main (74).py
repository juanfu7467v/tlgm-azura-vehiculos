...
def parse_messages_in_order(messages: list) -> dict:
    """
    Parsea cada mensaje por separado y fusiona los resultados
    respetando el orden original de llegada:
      - La posicion de cada clave queda definida por el PRIMER mensaje
        en el que aparece.
      - Los valores de mensajes posteriores se acumulan con ' | '.
    """
    merged: dict[str, str] = {}
    for msg_obj in messages:
        raw = (msg_obj.get("message") or "").strip()
        if not raw:
            continue
        msg_parsed = universal_parser(raw)
        for key, val in msg_parsed.items():
            if key in merged:
...
def build_response(messages: list, file_urls: list) -> dict:
    ...
    # Usar parse_messages_in_order para respetar el orden de llegada
    parsed   = parse_messages_in_order(messages)
...
# ── Mantener el orden de inserción en el JSON (no ordenar alfabéticamente) ──
